...
.. function:: chmod(path, mode; recursive=false)
   ...
   Change the permissions mode of ``path`` to ``mode``\ . Only integer ``mode``\ s (e.g. 0o777) are currently supported. If recursive=true and the file is a directory all permissions in that directory will be recursively changed.
