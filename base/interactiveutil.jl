...
downloadcmd = nothing
@unix_only function download(url::AbstractString, filename::AbstractString, silent::Bool)
    global downloadcmd
    ...
    elseif downloadcmd == :curl
        if silent
            run(`curl -silent -o $filename -L $url`)
        else
...
@windows_only function download(url::AbstractString, filename::AbstractString, silent::Bool)
...
function download(url::AbstractString; silent::Bool=false)
    """Downloads the webpage at url and saves it to a temporary file.
    The path to the temp file is returned.

    Platform specific details:
    In Linux / OS X, passing silent=false will suppress the progress messages generated by
    wget and curl.

    In windows, download requires a full URL - so passing xkcd.com fails, but
    http://xkcd.com succeeds. Additionally, the windows download function is always silent,
    as no progress message is implemented.
    """

    filename = tempname()
    download(url, filename, silent)
end
