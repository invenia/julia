...
downloadcmd = nothing
@unix_only function download(url::AbstractString, filename::AbstractString; silent::Bool=true)
    global downloadcmd
...
function download(url::AbstractString; silent::Bool=true)
    filename = tempname()
    @unix ? download(url, filename, silent=silent) : download(url, filename)
end
