...
static void finalize_object(arraylist_t *list, jl_value_t *o, arraylist_t *tmp)
{
    for(int i = 0; i < list->len; i+=2) {
        if (o == (jl_value_t*)list->items[i]) {
            ...
            list->len -= 2;
            arraylist_push(tmp, o);
            arraylist_push(tmp, f);
        }
    }
}
...
void jl_finalize(jl_value_t *o)
{
    ...
    arraylist_t tmp;
    arraylist_new(&tmp, 0);
    arraylist_push(&tmp, NULL); // GC frame size to be filled later
    arraylist_push(&tmp, NULL); // pgcstack to be filled later
